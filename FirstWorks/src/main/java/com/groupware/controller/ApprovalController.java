...
//	휴가신청서 입력하기 (휴가신청서 테이블, 전자결재 테이블, 결재대기함 동시에 입력)
	...
	public String apv_vc_pro(Apv_vc_dto vc, HttpSession session, Model m) {
		m.addAttribute("mem_no", (Integer) session.getAttribute("mem_no"));
		m.addAttribute("dept_no", (Integer) session.getAttribute("dept_no"));
...
//	대기테이블에서 휴가 상세보기 가져오기
	...
	public String apv_wait_detail(int apv_no, Model m, HttpSession session) {
		...
		return "/Apv_wait_detail";
	}

//	대기테이블에 중간결재자 업데이트
//	휴가테이블에 중간결재자 업데이트(휴가 본문에 도장 업데이트문)
	@PostMapping("/Apv_update1")
	public String apv_update1(HttpSession session, int apv_no) {
...
//	대기테이블에서 삭제(완료테이블로 가기 위해)
	...
	public String apv_update2(HttpSession session, int apv_no) {
...
//	결재완료된것들 리스트 페이지 불러오기(자기가 결재한 문서들만)
	...
	public String apv_cf_list(HttpSession session, Model m) {
...
//	결재완료된것들 리스트 전체 불러오기
	...
	public String apv_cf_list_all(Model m) {
...
//	반려처리함 리스트 페이지 불러오기
	@GetMapping("/Apv_rjt")
	public String apv_rjt(Apv_vc_dto vc) {
		service.apv_rjt(vc);
		return "redirect:/apv_wait_list";
	}
